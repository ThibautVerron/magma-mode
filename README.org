...
2. If needed, install the required packages [[https://github.com/rejeep/f.el][f.el]] (filenames library)
   and [[https://github.com/magnars/dash.el][dash.el]] (lists library). 
...
Keymap for the =magma-mode=:
 ...
 - =C-c C-e= (=magma-eval=): if region is set, send the region to the
   magma process, else send the current expression
 - =C-c C-f= (=magma-eval-defun=): evaluate the current function definition
...
Additionally, the evaluation functions take optional prefix arguments:
...
Additionally, if you have =magma-use-electric-newline= set to =t=,
